...
def prepare_model_for_vllm(model_args: ModelConfig, tokenizer_or_processor, is_peft: bool) -> str:
    """
    ...
    if is_peft:
        ...
        model = AutoPeftModelForCausalLM.from_pretrained(ft_path, device_map="cuda" if torch.cuda.is_available() else "cpu")
        ...
        tokenizer_or_processor.save_pretrained(tmp_dir)
        ...
        time.sleep(2)
        logger.info(f"sleeping to allow memory free up")
        return tmp_dir
...
def inference_function(model_args: ModelConfig, script_args: ScriptArguments, training_args: SFTConfig) -> None:
    ...
    tokenizer_or_processor = load_tokenizer_or_processor(script_args, model_args)
    ...
    if getattr(model_args, "use_peft", False):
        ft_path = prepare_model_for_vllm(model_args, tokenizer_or_processor, is_peft=True)
    else:
        ft_path = prepare_model_for_vllm(model_args, tokenizer_or_processor, is_peft=False)

...
kubectl cp ./option-2-supervised-fine-tuning fsx-upload:/data/shared
```
...
```yaml
output_dir: "/data/shared/option-2-supervised-fine-tuning/model/"
train_dataset_path: "/data/shared/option-2-supervised-fine-tuning/data/train/"
val_dataset_path: "/data/shared/option-2-supervised-fine-tuning/data/val/"
```
...
command:
  ...
  - -c
  - |
    pip install --no-cache-dir -r /data/shared/option-2-supervised-fine-tuning/requirements.txt && \
    torchrun \
    --nnodes=1 \
    --nproc_per_node=4 \
    --rdzv_backend=c10d \
    --rdzv_endpoint=${PET_RDZV_ENDPOINT} \
    --rdzv_id=qwen3-4b-instruct-sft \
    --max_restarts=3 \
    /data/shared/option-2-supervised-fine-tuning/scripts/train.py \
    --config /data/shared/option-2-supervised-fine-tuning/args.yaml
```
...
# Check logs
kubectl logs -f qwen3-4b-instruct-sft-worker-0

# Check PyTorchJob status
kubectl get pytorchjob qwen3-4b-instruct-sft
```
...
kubectl describe inferenceendpointconfig qwen3-4b-instruct-sft-endpoint
...
kubectl delete inferenceendpointconfig qwen3-4b-instruct-sft-endpoint

# Delete the training job
kubectl delete pytorchjob qwen3-4b-instruct-sft

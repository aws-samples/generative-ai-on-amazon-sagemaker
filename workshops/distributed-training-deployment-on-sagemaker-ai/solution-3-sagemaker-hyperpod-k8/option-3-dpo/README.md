...
kubectl cp ./option-3-dpo fsx-upload:/data/shared
```
...
```yaml
output_dir: "/data/shared/option-3-dpo/model/"
train_dataset_path: "/data/shared/option-3-dpo/data/train/"
val_dataset_path: "/data/shared/option-3-dpo/data/val/"
```
...
command:
  ...
  - -c
  - |
    pip install --no-cache-dir -r /data/shared/option-3-dpo/requirements.txt && \
    torchrun \
    --nnodes=1 \
    --nproc_per_node=4 \
    --rdzv_backend=c10d \
    --rdzv_endpoint=${PET_RDZV_ENDPOINT} \
    --rdzv_id=qwen3-4b-instruct-dpo \
    --max_restarts=3 \
    /data/shared/option-3-dpo/scripts/train.py \
    --config /data/shared/option-3-dpo/args.yaml
```
...
# Check logs
kubectl logs -f qwen3-4b-instruct-dpo-worker-0

# Check PyTorchJob status
kubectl get pytorchjob qwen3-4b-instruct-dpo
```
...
kubectl describe inferenceendpointconfig qwen3-4b-instruct-dpo-endpoint
...
kubectl delete inferenceendpointconfig qwen3-4b-instruct-dpo-endpoint

# Delete the training job
kubectl delete pytorchjob qwen3-4b-instruct-dpo

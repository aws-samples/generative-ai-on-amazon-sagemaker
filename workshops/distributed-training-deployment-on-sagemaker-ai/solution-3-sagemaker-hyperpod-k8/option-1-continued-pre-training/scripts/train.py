...
def prepare_dataset(
    ...
    if script_args.modality_type == "image":
        logger.info(
            "Image modality: skipping pre-tokenization and max_length auto-calc "
            "(handled by the data collator). "
            f"Truncation={script_args.apply_truncation}, max_length={script_args.max_length}"
        )
        if script_args.apply_truncation and script_args.max_length is None:
            logger.warning(
                "apply_truncation=True but max_length is unset for image modality; "
                "set max_length explicitly to truncate multi-modal sequences."
            )
        return train_ds, test_ds
